...
def main():
    ...
    while numero_maquinas != 0:
        ...
        for i in range(combates):
            ...
            criticos[i] = {}
            for ___ in range(combates):
                criticos[i]['criticos'] = 0
                criticos[i]['coordenada'] = ()

        # combate comça
        numero_criticos_totais = 0
            ...
            for atk in range(3):
                ...
                if maquinas[int(ataque[0])]['vida'] > 0:
                    ...
                    if (int(ataque[3]), int(ataque[4])) == maquinas[int(ataque[0])][ataque[1]]['coordenadas']:  # "critico verdadeiro"
                        criticos[int(ataque[0])]['criticos'] += 1
                        criticos[int(ataque[0])]['coordenada'] = (int(ataque[3]), int(ataque[4]))
                        numero_criticos_totais += 1
        ...
        if vida_aloy > 0 and final != "Aloy ficou sem flechas e recomeçará sua missão mais preparada.":
            ...
            # críticos
            if numero_criticos_totais > 1:
                print("Críticos acertados:", sep='')
                for n in criticos:
                    if criticos[n]['criticos'] > 0:
                        print("Críticos acertados:", sep='')
            ...
            if numero_criticos > 0:
                ...
                for n in criticos:
                    ...
                    spots = []
                    spots_conjunto = set()
                    for ponto in list(criticos[n].values()):
                        spots.append(list(criticos[n].values()).count(ponto))
