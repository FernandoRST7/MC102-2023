...
def completa_com_zero(vet: List[int], L: List[int]) -> List[int]:
    if len(vet) > len(L):
        i = len(vet) - len(L)
        while i != 0:
            L.append(0)
...
def completa_com_um(vet: List[int], L: List[int]) -> List[int]:
    if len(vet) > len(L):
        i = len(vet) - len(L)
        while i != 0:
            L.append(1)
...
# funções:
def soma_vetores(vet: List[str], vet_comand: List[int]) -> List[int]:
    completa_com_zero(vet, vet_comand)
...
def multiplica_vetores(vet: List[int], vet_comand: List[int]) -> List[int]:
    completa_com_um(vet, vet_comand)
    for i in range(len(vet)):
        vet[i] = vet[i] * vet_comand[i]
    return vet
...
def n_duplicacao(vet: List[int], n: List[int]) -> List[int]:
    if n == 0:
        vet = []
        return vet
    else:
...
def produto_interno(vet: List[int], vet_comand: List[int]) -> List[int]:
