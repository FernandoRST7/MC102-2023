...
for g in range(D):
    ...
    #contagem das brigas:
    quantas_brigas = 0
    for i in range(len(Lista_animais_presentes)):
       for j in range(len(Lista_briguentos)//2):
            if Lista_animais_presentes[i] == Lista_briguentos[j*2]:
                for k in range(len(Lista_animais_presentes)):
                    if Lista_animais_presentes[k] == Lista_briguentos[j*2+1]:
                        quantas_brigas += 1
